...
import {
  ...
  TrendingUp, Home, CreditCard, Star, Video, Upload
} from 'lucide-react';
...
const studentNav: NavItem[] = [
  ...
  { label: 'Live Classes', href: '/dashboard/live', icon: <Video className="w-4 h-4" /> },
...
const instructorNav: NavItem[] = [
  ...
  { label: 'Content Upload', href: '/instructor/content', icon: <Upload className="w-4 h-4" /> },
  { label: 'Live Classes', href: '/instructor/live', icon: <Video className="w-4 h-4" /> },
...
const adminNav: NavItem[] = [
  ...
  { label: 'Live Classes', href: '/admin/live', icon: <Video className="w-4 h-4" /> },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const roleColor = currentUser?.role === 'admin' ? 'var(--amber)' : currentUser?.role === 'instructor' ? 'var(--accent2)' : 'var(--accent)';
      ...
      <div className="p-5 border-b" style={{ borderColor: 'var(--border-color)' }}>
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--bg-card)' }}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
            style={{ background: `linear-gradient(135deg, ${roleColor}, var(--accent2))` }}>
